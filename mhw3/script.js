const ClientID = '9056e8a07e564f5eb78151fe419c5767';
const ClientSecret = 'c9fb854392464f1b8291bf5bf79c94ae';
const key = 'AIzaSyCU5OwCgYEeCrXjksIiSjvdcOEXEjZGft8';
let token;
...
function ottieniCover(json, immagine, descrizione) {
    immagine.src = json.tracks.items[0].album.images[0].url;
...
function ottieniCopertina(json, immagine, descrizione) {
    immagine.src = json.items[0].volumeInfo.imageLinks.thumbnail;
    descrizione.textContent = json.items[0].searchInfo.textSnippet;
}
...
for (const content of contents) {
    ...
    */
    ...
    if (content.categoria == "Musica") {
        if (!token) {
            const richiesta = new XMLHttpRequest();
            richiesta.addEventListener("load", reqListener);
            richiesta.open("POST", 'https://accounts.spotify.com/api/token', false);
            richiesta.setRequestHeader("Content-Type", 'application/x-www-form-urlencoded');
            richiesta.setRequestHeader("Authorization", 'Basic ' + btoa(ClientID + ':' + ClientSecret));
            richiesta.send('grant_type=client_credentials');
        }
        fetch('https://api.spotify.com/v1/search?type=track&q=' + content.nome, {
            headers: {
                'Authorization': 'Bearer ' + token
            }
        }).then(onResponse).then((json) => ottieniCover(json, immagine, descrizione));


    } else if (content.categoria == "Libri") {
        fetch('https://www.googleapis.com/books/v1/volumes?printType=books&key=' + key + '&q=' + content.nome)
            .then(onResponse).then(json => ottieniCopertina(json, immagine, descrizione));
    }
